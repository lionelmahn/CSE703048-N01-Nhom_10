...
<div class="row">
    <div class="col-md-8 mx-auto">
        <h1 class="h3 mb-4">Tạo Chương trình Đào tạo</h1>
        
        <div class="card">
            <div class="card-body">
                <form action="{{ route('ctdt.store') }}" method="POST">
                    @csrf
                    
                    <div class="mb-3">
                        <label for="ma_ctdt" class="form-label">Mã CTĐT <span class="text-danger">*</span></label>
                        <input type="text" class="form-control @error('ma_ctdt') is-invalid @enderror" id="ma_ctdt" name="ma_ctdt" value="{{ old('ma_ctdt') }}" required>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('he_dao_tao_id') is-invalid @enderror" id="he_dao_tao_id" name="he_dao_tao_id" required>
                            ...
                            <option value="{{ $he->id }}" @selected(old('he_dao_tao_id') == $he->id)>{{ $he->ten }}</option>
                            @endforeach
                        </select>
                        @error('he_dao_tao_id')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    
                    {{-- Added Nganh field (required) --}}
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('nganh_id') is-invalid @enderror" id="nganh_id" name="nganh_id" required>
                            ...
                            <option value="{{ $nganh->id }}" @selected(old('nganh_id') == $nganh->id)>{{ $nganh->ten }}</option>
                            @endforeach
                        </select>
                        @error('nganh_id')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    
                    {{-- Added ChuyenNganh field (optional) --}}
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('chuyen_nganh_id') is-invalid @enderror" id="chuyen_nganh_id" name="chuyen_nganh_id">
                            ...
                            <option value="{{ $chuyenNganh->id }}" @selected(old('chuyen_nganh_id') == $chuyenNganh->id)>{{ $chuyenNganh->ten }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('nien_khoa_id') is-invalid @enderror" id="nien_khoa_id" name="nien_khoa_id" required>
                            ...
                            <option value="{{ $nk->id }}" @selected(old('nien_khoa_id') == $nk->id)>{{ $nk->ma }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control @error('hieu_luc_tu') is-invalid @enderror" id="hieu_luc_tu" name="hieu_luc_tu" value="{{ old('hieu_luc_tu') }}" required>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control @error('hieu_luc_den') is-invalid @enderror" id="hieu_luc_den" name="hieu_luc_den" value="{{ old('hieu_luc_den') }}">
